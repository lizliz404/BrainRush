...
- 新增 60 秒快速练习模式（固定 Normal 参数 + 无限命），更适合低门槛体验。
- 新增 **Word Speedrun（单词极速跑）** 分支：中英互选 + 句子填空（随机混合）。
- 角色外观自定义与分数解锁。
- 本地保存最高分和角色外观。
...
- 最高分、角色外观、题目调节参数都使用 `localStorage` 持久化，并在 `useEffect` 中按需写回。
...
- 主菜单与结算页使用低音量 Web Audio 原生振荡器 BGM，进入页面 1 秒后淡入。
- 开始游戏时自动淡出菜单 BGM，降低音频切换突兀感。
